...
def get_overview_prompt_data(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> pd.DataFrame:
    """
    Get overview prompt data with enhanced prompts for analytics.
    Excludes test users and system test data.
    ...
    Returns:
        DataFrame with prompt and enhancement data
    """
    ...
    LEFT JOIN public.refine_prompt rp
        ON up.prompt_id = rp.prompt_id OR sep.enhanced_prompt_id = rp.enhanced_prompt_id
    {where_clause}
    ...
    try:
        ...
        logger.info(f"Overview prompt data query returned {len(df)} rows")
        return df
    except Exception as e:
        logger.error(f"Overview prompt data query failed: {e}")
        return pd.DataFrame()
...
def get_dau_wau_mau(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    WITH daily_active AS (
        SELECT COUNT(DISTINCT COALESCE(sep.user_id, up.user_id)) AS dau
    ...
    weekly_active AS (
        SELECT COUNT(DISTINCT COALESCE(sep.user_id, up.user_id)) AS wau
        FROM public.save_enhance_prompt sep
        LEFT JOIN public.user_prompts up ON sep.prompt_id = up.prompt_id
        WHERE sep.created_at >= CURRENT_DATE - INTERVAL '7 days'
    ),
    monthly_active AS (
        SELECT COUNT(DISTINCT COALESCE(sep.user_id, up.user_id)) AS mau
        FROM public.save_enhance_prompt sep
        LEFT JOIN public.user_prompts up ON sep.prompt_id = up.prompt_id
    ...
    try:
        ...
        if not df.empty:
            return {
                "dau": int(df.iloc[0]['dau'] or 0),
                "wau": int(df.iloc[0]['wau'] or 0),
                "mau": int(df.iloc[0]['mau'] or 0),
                "dau_mau_ratio": float(df.iloc[0]['dau_mau_ratio'] or 0.0)
            }
        return {"dau": 0, "wau": 0, "mau": 0, "dau_mau_ratio": 0.0}
    except Exception as e:
...
def get_feature_usage_counts(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> pd.DataFrame:
    ...
    try:
        ...
        # Build refine condition using rp.created_at instead of sep.created_at
        refine_date_condition = build_date_where_clause("rp.created_at", start_date, end_date)
        refine_where = f"WHERE {refine_date_condition}" if refine_date_condition else ""
...
def get_token_consumption(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> pd.DataFrame:
    ...
    and_clause = f"AND {date_condition}" if date_condition else ""
    ...
    try:
        ...
        """
        df = db_manager.execute_query(query.format(and_clause=and_clause))
        return df
    except Exception as e:
...
@st.cache_data(ttl=300)
def get_prompt_generation_metrics(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict[str, Any]:
    """
    ...
    """
    if not DATABASE_AVAILABLE:
        return {"total_prompts": 0, "prompts_per_user": 0.0, "unique_users": 0}
    ...
    SELECT 
        ...
        ROUND(COUNT(*)::decimal / NULLIF(COUNT(DISTINCT up.user_id), 0), 2) AS prompts_per_user
    FROM public.user_prompts up
    {where_clause}
    """
    ...
    try:
        ...
        if not df.empty:
            return {
                ...
                "unique_users": int(df.iloc[0]['unique_users'] or 0)
            }
        return {"total_prompts": 0, "prompts_per_user": 0.0, "unique_users": 0}
    except Exception as e:
...
def get_acquisition_kpis(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict[str, Any]:
    """
    ...
    if start_date and end_date:
        ...
        prev_users = int(prev_df.iloc[0]['prev'] or 0) if not prev_df.empty else 0
...
def get_active_user_summary() -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with active user metrics
    """
    dau_wau_mau = get_dau_wau_mau()
    return {
        "dau": dau_wau_mau.get("dau", 0),
        "wau": dau_wau_mau.get("wau", 0),
        "mau": dau_wau_mau.get("mau", 0),
        "stickiness": dau_wau_mau.get("dau_mau_ratio", 0.0)
    }
...
@st.cache_data(ttl=600)
def get_engagement_score() -> float:
    """
    ...
    """
    # Simple engagement score based on active users and usage
    dau_wau_mau = get_dau_wau_mau()
    dau = dau_wau_mau.get("dau", 0)
    mau = dau_wau_mau.get("mau", 0)
    
    if mau > 0:
        # Score based on DAU/MAU ratio and absolute DAU
        stickiness_score = min(dau_wau_mau.get("dau_mau_ratio", 0.0), 100.0)
        activity_score = min((dau / 1000) * 50, 50.0)  # Scale based on DAU
        return round((stickiness_score + activity_score) / 2, 2)
    return 0.0
