...
export async function GET(request) {
  try {
    ...
    const dailyHabitData = calculateDailyHabitUsers(allData, periodData, endDate);
    
    // Generate daily trend data
    const dailyTrendData = generateDailyTrendData(allData, periodData, endDate);
    const dormantTrendData = generateDormantTrendData(allData, periodData, endDate);
    ...
    console.log('Trend Data Generated:', {
      ...
      dormantTrendWithActivity: dormantTrendData.filter(d => d.total > 0).length
    });
...
function calculateDailyHabitUsers(allData, periodData, analysisEndDate) {
  ...
  const highVolumeUsers = Object.keys(promptsPerUser).filter(
  ...
  allData.forEach(row => {
    if (!row.user_id || !row.prompt_created_at) return;
    if (!highVolumeUsers.includes(row.user_id)) return;
    
    const dateStr = new Date(row.prompt_created_at).toISOString().split("T")[0];
    if (!distinctDaysPerUser[row.user_id]) {
      distinctDaysPerUser[row.user_id] = new Set();
    }
    distinctDaysPerUser[row.user_id].add(dateStr);
  });
  ...
  const distinctDaysArray = Object.values(distinctDaysCount).sort((a, b) => a - b);
  ...
  periodData.forEach(row => {
    ...
    if (rowDate >= sevenDaysAgo) {
      recentlyActiveUsers.add(row.user_id);
    }
  });
  ...
  allData.forEach(row => {
    if (row.user_id && !userStatusMap[row.user_id]) {
      userStatusMap[row.user_id] = row.user_status || 'free';
  ...
  const segmentUsers = (userIds) => {
    ...
    userIds.forEach(userId => {
      ...
      if (status.includes('pro') || status.includes('paid') || status.includes('premium')) {
        segments.pro++;
...
function generateDailyTrendData(allData, periodData, endDate) {
  ...
  Object.keys(dataByDate).forEach(dateStr => {
    ...
    const usersActiveToday = new Set(dayData.map(row => row.user_id));
...
function generateDormantTrendData(allData, periodData, endDate) {
  ...
  allData.forEach(row => {
    ...
    if (!userStatusMap[row.user_id]) {
      userStatusMap[row.user_id] = row.user_status || 'free';
    }
    
    const rowDate = new Date(row.prompt_created_at);
    if (!userLastActive[row.user_id] || rowDate > userLastActive[row.user_id]) {
      userLastActive[row.user_id] = rowDate;
    }
  });
