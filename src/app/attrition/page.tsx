...
export default function AttritionPage() {
  ...
  const { data, isLoading } = useAttritionData(dateFilter, sourceFilter);
  ...
  // 2. Survival Curve
  // Calculate % active at day X
  // Get all lifespans (active users are censored, but for simple curve we check who survived past day X)
  // Actually, survival analysis uses Kaplan-Meier.
  // Approximation: For each day, look at users who started at least that many days ago.
  // Count how many are still active (daysSinceLastActive <= 30) OR lasted longer than X.

  // Simplified "Retention Curve": % of users making it to Day X.
  const maxDay = 90; // Plot up to 90 days
  const survivalData = [];
  for (let i = 0; i <= maxDay; i += 7) {
    // Users who have "lived" at least i days (lifespan >= i)
    // AND are NOT churned before day i.
    // Actually, "Retention at Day X" = (Users with lifespan >= X) / Total Users
    // This is simple retention behavior.
  ...
  // 3. Exit Triggers (Treemap) - REMOVED per user request
