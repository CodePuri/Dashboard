...
def date_filter(mode: str = "Developer") -> Tuple[str, Optional[datetime], Optional[datetime]]:
    ...
    if mode == "Business":
        preset = st.selectbox(
            "Time Period",
            ["Today", "Tomorrow", "Last 7 Days", "Last 30 Days", "Last 90 Days", "Custom"],
        ...
        if preset == "Today":
            ...
            with col1:
                start_date = st.date_input("Start Date", value=datetime.now() - timedelta(days=30), key="business_start_date")
            with col2:
                end_date = st.date_input("End Date", value=datetime.now(), key="business_end_date")
...
def section_date_filter(section_key: str, default_preset: str = "Last 7 Days") -> Tuple[str, Optional[datetime], Optional[datetime]]:
    """
    Create a date filter for a specific section.
    
    Args:
        section_key: Unique key for this section (to avoid widget conflicts)
    ...
    preset = st.selectbox(
        "📅 Date Range",
        ["Today", "Tomorrow", "Last 7 Days", "Last 30 Days", "Last 90 Days", "Custom"],
        index=["Today", "Tomorrow", "Last 7 Days", "Last 30 Days", "Last 90 Days", "Custom"].index(default_preset) if default_preset in ["Today", "Tomorrow", "Last 7 Days", "Last 30 Days", "Last 90 Days", "Custom"] else 2,
        key=f"section_date_{section_key}"
    )
    ...
    if preset == "Today":
        ...
        col1, col2 = st.columns(2)
        with col1:
            start_date = st.date_input("Start Date", value=datetime.now() - timedelta(days=30), key=f"section_start_{section_key}")
        with col2:
            end_date = st.date_input("End Date", value=datetime.now(), key=f"section_end_{section_key}")
        start_date = datetime.combine(start_date, datetime.min.time())
        end_date = datetime.combine(end_date, datetime.max.time())
